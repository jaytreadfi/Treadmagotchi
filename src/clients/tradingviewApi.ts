/**
 * TradingView client — fetches technical analysis via the scanner API.
 * Ported from treadbot/backend/app/clients/tradingview.py.
 */
import { PROXY_BASE } from '@/lib/constants';

// Map base symbols to TradingView BINANCE perp symbols
...
// The columns we request from TradingView scanner
const COLUMNS = [
  'Recommend.All',        // Overall recommendation score (-1 to +1)
  'Recommend.All|1',      // # of buy signals
  'RSI',
  'MACD.signal',
  'EMA20',
  'SMA50',
  ...
  'change',               // % change
  'volume',               // 24h volume
  'Recommend.MA',         // MA recommendation
...
export async function getAnalysis(pairs: string[]): Promise<Record<string, TVAnalysis>> {
  ...
  try {
    const res = await fetch(`${PROXY_BASE}/tradingview`, {
      method: 'POST',
    ...
    for (let i = 0; i < (data.data || []).length; i++) {
      ...
      // values[1] is Recommend.All|1 (not directly buy count, but we can derive)
      // For buy/sell/neutral counts, we approximate from the score
      const total = 26; // TradingView uses 26 indicators
...
export function toContextString(analyses: Record<string, TVAnalysis>): string {
  ...
  for (const [symbol, d] of entries) {
    ...
    // Determine trend from EMA/SMA crossover
    let trend = 'N/A';
